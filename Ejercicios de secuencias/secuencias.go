...
ACCION ejercicio26 ES
	...
	PROCESO
		...
		MIENTRAS NFDS(s1) Y NFDS(s2) HACER:
			...
			car:= v1;  //guardo la prier letra de la secuencia1

			MIENTRAS NFDS(s1) HACER:
				MIENTRAS (v <> " ") HACER:
					car2:= v1; //guardo el siguiente caracter, hasta estar en el ultimo 
					AVZ(s1,v1);
				Fin_mientras;
			
				SI (car1 = car2) ENTONCES: //comparo el primer caracter y el ultimo
					GRABAR(sal,v1);
					AVZ(s1,v1);
				Fin_si;
				GRABAR(sal," ");
